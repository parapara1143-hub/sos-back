...
def mtta_mttr():
    ...
    if cid is not None:
        q_inc = q_inc.filter(Incident.company_id == cid)
    ...
    for inc in incs:
        ...
        if t_created and t_first_action:
            total_a += (t_first_action - t_created).total_seconds()
            count_a += 1

        if t_created and t_resolved:
            total_r += (t_resolved - t_created).total_seconds()
            count_r += 1
    ...
    return {
        ...
        "mtta_sec": int(mtta),
        "mttr_sec": int(mttr),
        "count_actions": count_a,
        "count_resolved": count_r
    }
